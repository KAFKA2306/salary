...
def build_catalog(manifest: dict[str, Any]) -> dict[str, Any]:
    """Return the UI catalog and conservative aggregate eligibility.

    Artifact paths come exclusively from the manifest. UNKNOWN_PROVENANCE rows
    are visible for audit but never counted as current/canonical datasets.
    Byte-identical aliases share one blob group and therefore cannot be counted
...
def inspect_csv_bytes(raw: bytes) -> dict[str, Any]:
    """Inspect a CSV snapshot without changing or interpreting its values."""
    encoding = "utf-8-sig"
    try:
...
def inspect_payload(manifest: dict[str, Any], selected_path: str, raw: bytes | None = None) -> dict[str, Any]:
    """Inspect one manifest-registered artifact and return audit-only metadata."""
    ...
    same_blob = sorted(
        item["path"] for item in catalog["catalog"] if item["git_blob_sha"] == selected["git_blob_sha"]
    )
    detail: dict[str, Any] = {}
...
async def inspect_same_origin(manifest_url: str, artifact_url: str, selected_path: str) -> str:
    """Pyodide entrypoint. Fetch only URLs supplied by the same-origin worker."""
